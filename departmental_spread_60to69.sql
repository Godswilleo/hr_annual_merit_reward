...
WHERE 
  k.staff_id IN (
    ...
    FROM 
      kpi_scores
	WHERE
	total_kpi_scores BETWEEN 60 AND 69
  )
  )
  SELECT department, count(total_kpi_scores BETWEEN 60 AND 69) AS 'number of staff' FROM cte_dept_spread GROUP BY department
